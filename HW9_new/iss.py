...
def getIssKeyPointIdx(filename, thre1=0.6,thre2=0.6,r1=2,r2=2,iss_count=500):
    ...
    """Extract the iss pcd from the src pcd
    """
def extract_iss_pcd(pcd_src):   
    pcd = copy.deepcopy(pcd_src)
    iss_idxs =getIssKeyPointIdxFromPcd(pcd)
    points = np.array(pcd.points)
    iss_points = points[iss_idxs]
    pcd.points =o3d.utility.Vector3dVector(iss_points)
    pcd.estimate_normals(o3d.geometry.KDTreeSearchParamHybrid(radius=1, max_nn=20))
    return pcd
"""
The implementation of iss key point selection algorithm
"""
def getIssKeyPointIdxFromPcd(point_cloud_pynt, thre1=0.6,thre2=0.6,r1=2,r2=2,iss_count=500):
    ...
    #point_cloud_o3d = point_cloud_pynt.to_instance("open3d", mesh=False)
    points = point_cloud_pynt.points
    pcd_tree = o3d.geometry.KDTreeFlann(point_cloud_pynt)
    points =np.array(points)
    n=points.shape[0]
    normals = np.zeros((n,3))
    lambda_dict={}
...
if __name__ == "__main__":
    ...
    print(getIssKeyPointIdxFromPcd(src_down))
